...
Мы можем передать в функцию произвольное количество позиционных аргументов, для этого существует общепринятый синтаксис:
...
**Важно:** `*args` превращает некоторую переданную коллекцию аргументов в `tuple`

**Важно:** одиночная звезда `*` распаковывает коллекцию в позиционные аргументы.  

*Примечание:* вместо `args` может быть написано любое другое слово.

## Отгрызаем аргумент от args

Мы можем "отгрызть" несколько элементов от `args`, если нам необходим такой сценарий работы (по умолчанию функция с одним единственным аргументом - `*args` можем принимать произвольное кол-во аргументов):
...
Теперь функция будет требовать как минимум один обязательный позиционный аргумент `arg`.

## Передача произвольного количества ключевых аргументов

Мы можем передать в функцию произвольное количество ключевых аргументов, для этого существует общепринятый синтаксис:
...
**Важно:** `**kwargs` превращает переданную последовательность ключевых аргументов в `dict`

**Важно:** двойная звезда `**` делает то же самое, что и одиночная, только распаковывает коллекцию в виде словаря.  

*Примечание:* вместо `kwargs` может быть написано любое другое слово.

## Указываем обязательные ключевые аргументы

Если нам необходимо указать, что, начиная с некоторого аргумента, функция должна принимать только ключевые, необходимо указать:
...
foo(1, 2, 3, 4) # ошибка
foo(1, 2, c=3, d=4) # правильный вызов функции
```

После символа `*` все аргументы будут требовать синтаксис ключевых при вызове функции.

## Анонимные функции. lambda.

Лямбда-функция в Python — это просто функция Python. Но это некий особенный тип `lambda` с ограниченными возможностями. Лямбда функция является элементов функциональной парадигмы программирования, которую, кстати, Python поддерживает в некотором роде. 

Синтаксис:

```python
lambda аргументы: тело_функции
```

Рассмотрим некоторую функцию `map` (к слову она из той же серии про функциональное программирование). Эта функция принимает два аргумента:

* Некоторую функцию
* Коллекцию

А затем применяет принятую функцию к каждому элементу коллекции поэлементно. Вот небольшой пример использования `map`:

```python
a = ['Hello', 'world']
a = list(map(len, a))  # Применяем функцию len к каждому элементу списка а
print(a) # Выведет [5, 5]
```

**Подробнее про функцию `map` поговорим немного позже** 

А теперь рассмотрим такой сценарий:

Необходимо каждый элемент целочисленного списка умножить на 2, а затем увеличить на 10. Это мы можем сделать с помощью цикла:
...
for i in range(len(a)):
    a[i] *= 2
    a[i] += 2
print(a) # выведет: [12, 14, 16, 18, 20]
```

А теперь сделаем то же самое с помощью `lambda`:

```python
a = [1, 2, 3, 4, 5] # наш список
a = list(map(lambda x: x*2 + 10, a))
print(a) # выведет: [12, 14, 16, 18, 20]
```
